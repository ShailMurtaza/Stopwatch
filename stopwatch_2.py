...
def timer_window():
    timer_tk = Tk()
    timer_tk.title("TIMER")
    timer_tk.geometry("250x50")
    timer_tk.configure(bg='white')
    timer_tk.resizable(0, 0)
    timer_tk.wm_attributes("-topmost", 1)
    global timer_window_time
    timer_window_time = Label(timer_tk, font=("Helvetica", 20), text="Welcome", fg="red", bg="white")
    timer_window_time.pack()
    return timer_tk
...
def reset():
    ...
    if not run:
        ...
        reset_button['state'] = "active"
    mark['text'] = '00:00:00'
    timer_window_time["text"] = '00:00:00'
    with open('stopwatch', "w") as f:
...
timer_tk = timer_window()
# global timer_tk
if h != "0" and m != "0" and s != "0":
    time = "{}:{}:{}".format(h, m, s)
    mark['text'] = time
    timer_window_time["text"] = time
    reset_button['state'] = "active"
    del time
...
timer_tk.protocol("WM_DELETE_WINDOW", on_closing)
root.mainloop()
